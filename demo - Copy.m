...
%     disp(biasias );
    ...
    function [outputs] = calculate_hiddenLayer_outputs(inputs, weights, biasias)
          input_samples_count = size(training_input,1);
          weight_size = size(weights);
          outputs = zeros(input_samples_count, 3);
          for i=1:1:(input_samples_count)
            for j=1:1:(weight_size -1)
             outputs(i, :) = outputs(i, :) + inputs(i, :).*weights(j, :)+biasias(j, :);
            end
          end
    end
...
%             disp(loss);
    ...
    for i=1:1:1000
        hiddenLayerResults = calculate_hiddenLayer_outputs(training_input, second_layer_weights, second_layer_biasias )
        
        [weight_update, biasias_update, loss] = calculate_updates(hiddenLayerResults, traing_result, weights, biasias);
